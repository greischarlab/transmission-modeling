...
We then fit a statistical model (Li & Handel, 2014) that estimates viral load 
values over time to the shedding data we just extracted using the least squares
method. We make the assumption that viral load is a 1-to-1 mapping to viral 
shedding.
```{r}
...
plot.viralload <- function(data, pars, sympdata, max.modeltime, yhigh, cex,
                           cex_labels, xlabel, ylabel, example, letter = "") {
  ...
  if (!example) {
    title <- paste(letter, ") Participant ", unique(data$participant),
                   ":\n", symp.timing, sep="")
  }
  ...
  axis(1, cex.axis=cex, at=seq(0, 400, by=100), labels = c(expression(0),
                                                          expression(100),
                                                          expression(200),
                                                          expression(300),
                                                          expression(400)),
  ...
  mtext(text=title, side=3, line=2, cex=cex_labels, font=2)

  mtext(text=ylabel, side=2, line=6, cex=cex_labels)
  
  mtext(text=xlabel, side=1, line=4.5, cex=cex_labels)
...
tiff(filename="Participant Trajectories.tiff", height=19, width=9.5,
    units="in", res=500)
par(mfrow=c(6, 3), mai=c(1, 1, 0.82, 0.5), oma=c(2, 3, 0, 3))
...
tiff(filename="Participant Trajectories ver 2.tiff", height=19, width=9.5,
    units="in", res=500)
par(mfrow=c(6, 3), mai=c(1, 1, 0.82, 0.5), oma=c(2, 3, 0, 3))
