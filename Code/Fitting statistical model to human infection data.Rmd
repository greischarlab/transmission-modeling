...
sse.viralLoad <- function(pars, data, detect.threshold, 
  ...
  if(!is.na(optimization) & optimization == T){
    ...
    # prevent negative infinity, which results from log10(0)
    # estimates[!is.finite(estimates) & estimates<0] <- (-16)
...
getPeak <- function(data){
  peakHour <- 24*data$days[which.max(data$y_virus)]
  return(peakHour)
  }

# tester functions to id timing of max viral load
participant.peak <- function(fits) {
    participants <- unique(fits$ID)
  
    participantparams <- do.call(rbind, 
                               lapply(participants, 
                                      function(x) getPeak(fits[fits$ID == x,])))
...
```{r, results='hide'}
# getLocalMin = function(data){
#   # obtains local minima of time series data
#   # and returns the corresponding index
#   # these are the times corresponding to places where the derivative
#   # switches from negative (decreasing) to positive (increasing)
#   minIndex = which(diff(sign(diff(data))) == 2) + 1
#   return(minIndex)
# }
# 
# first.local.min <- function(data, ptsEitherSide = 2) {
# 
#   #quantifiable.vl <- data[!is.na(data$y_virus),]$y_virus
#   #quantifiable.vl <- quantifiable.vl[3:length(quantifiable.vl)]
# 
#   nonmissingValues = which(is.finite(data$y_virus)|grepl("POS", data$y_type))
#   missingValues = which(is.na(data$y_virus)) # below detection limit
#   # ignore values before 5th finite value:
#   missingValues = missingValues[-which(missingValues<nonmissingValues[5])]
# 
#   minima <- getLocalMin(data$y_virus)#local.min.max(quantifiable.vl, plot = F)$minima
#   # ignore values before 5th finite value:
#   minima = minima[which(minima>=nonmissingValues[5])]
# 
#   localMinValue = NA
#   if(length(minima)>0){
#     res = length(minima)
#     isLM = rep(F, length(minima))
#     for (min.index in 1:length(minima)) {
# 
#       indexToConsider = c((minima[min.index]-ptsEitherSide):(minima[min.index]-1),
#                           (minima[min.index]+1):(minima[min.index]+ptsEitherSide))
#       if(all(data$y_virus[indexToConsider] > data$y_virus[minima[min.index]], na.rm = T)){
#         isLM[min.index] = T
#         } # end conditional: two values on either side of local minimum are greater?
#       } # end for loop thru local minima
#     if(any(isLM)){localMinValue = minima[isLM]}
#     } # end conditional: any local minima?
# 
#   res = length(data$y_virus)
#   if(length(missingValues)>0 | any(is.finite(localMinValue))){
#     res = min(c(missingValues,localMinValue),na.rm = T)
#     } # end conditional: any local minima or missing values?
# 
#   return(res)
# }
...
#   }
# 
# first.local.min <- function(data, daysAfter = 4) {
#   # 4DaysAfterSimplified
#   # stop at last value before abundance drops below detection threshold 
#     # for at least one day, or earlier for a local minimum that precedes
#     # daysAfter worth of larger counts
# 
...
#   missingValues = which(is.na(data$y_virus)) # below detection limit
#       # allMissingValues[which(gaps[allMissingValues]>=1)]
...
#     }
# 
#   localMinima = getLocalMin(data$y_virus)
#   if(length(localMinima)>0){
#     candidateMinima = unlist(lapply(1:length(localMinima), 
#                                       function(x) min(data$y_virus[which(data$days >= 
#                                                                            data$days[localMinima[x]] & data$days <= 
#                                                                            (data$days[localMinima[x]]+daysAfter))], 
#                                                  na.rm = T) == data$y_virus[localMinima[x]]))
#     if(any(candidateMinima)){endPt = min(localMinima[candidateMinima])}
#     } # end check for local minima
#   
#   return(endPt)
#   }
...
first.local.min <- function(data) {
  # ComboMethod
  # stop at last value before abundance drops below detection threshold
    # or earlier for a count below all subsequent counts prior to dropping below
    # detection threshold
  ...
  # check for values less than all subsequent values in detectable run
  ...
  # check for earlier end of infection wave
...
#
#   # MG added plot=F
#   minima <- local.min.max(quantifiable.vl, plot = F)$minima
#
#   for (mi in minima) {
#
...
```{r, eval = FALSE}
numFits <- 1000
# noro.fits.localmin <- all.fits(noro.FShedData[noro.FShedData$ID=="ID5",],
#                                noro.maxt.firstmin, numFits,
...
plot.viralload <- function(data, pars, sympdata, max.modeltime, cex=1,
  ...
  predValues <- sse.viralLoad(pars, data[1:max.modeltime,], optimization = F)
  linet <- predValues$t
  ...
  t.peak <- sse.viralLoad(pars, data[1:max.modeltime,], optimization = NA)[3]/24
  peakHeight <- sse.viralLoad(pars, data[1:max.modeltime,], optimization = NA)[1]
...
noro.fits.localmin$repRate <- log10(noro.fits.localmin$transp2)
noro.fits.localmin$peakDay <- noro.fits.localmin$transp3/24
...
sars.fits.localmin$repRate <- log10(sars.fits.localmin$transp2)
sars.fits.localmin$peakDay <- sars.fits.localmin$transp3/24
...
noro.PtsFitted <- do.call(rbind, 
                         lapply(noroParticipants, 
                                function(x) data.frame(x, 
                                                       positivePts((noro.FShedData[noro.FShedData$ID == x,])[1:noro.maxt.firstmin$max.point[noro.maxt.firstmin$ID==x],]))))
...
sars.PtsFitted <- do.call(rbind, 
                         lapply(sarsParticipants, 
                                function(x) data.frame(x, 
                                                       positivePts((sars.TShedData[sars.TShedData$ID == x,])[1:sars.maxt.firstmin$max.point[sars.maxt.firstmin$ID==x],]))))
colnames(sars.PtsFitted) <- c('ID', 'points.fit')

# MG suggestion:
noro.unclearpeaks <- c('ID6')

sars.unclearpeaks <- sars.PtsFitted$ID[sars.PtsFitted$points.fit < 5]#c('Throat_1', 'Throat_14')#c('Throat_1', 'Throat_12', 'Throat_13', 'Throat_14')
...
# previously referenced noro.params.localmin and sars.params,localmin
  # but we want to focus on the fits with clearpeaks only
noroSummary$peakDay <- lmMetrics(noro.params.clearpeaks, 'peakDay', noro.r.range)
noroSummary$symp <- lmMetrics(noro.params.clearpeaks, 'incubation', noro.r.range)
noroSummary$sympToPeakDay <- lmMetrics(noro.params.clearpeaks, 'symptomsToPeakDay', noro.r.range)
noroSummary$shedding <- lmMetrics(noro.params.clearpeaks, 'sheddingOnset', noro.r.range)
noroSummary$sympToShed <- lmMetrics(noro.params.clearpeaks, 'symptomsToStartShedding', noro.r.range)

sarsSummary$peakDay <- lmMetrics(sars.params.clearpeaks, 'peakDay', sars.r.range)
sarsSummary$symp <- lmMetrics(sars.params.clearpeaks, 'incubation', sars.r.range)
sarsSummary$sympToPeakDay <- lmMetrics(sars.params.clearpeaks, 'symptomsToPeakDay', sars.r.range)
sarsSummary$shedding <- lmMetrics(sars.params.clearpeaks, 'sheddingOnset', sars.r.range)
sarsSummary$sympToShed <- lmMetrics(sars.params.clearpeaks, 'symptomsToStartShedding', sars.r.range)
...
noro.params.localmin$avgTotalSheddingBeforeSymptoms <- noro.presymp.avg.totalshedding
```
