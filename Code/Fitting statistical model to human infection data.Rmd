...
Calculating average shedding and symptom onset for Figure 1
```{r}
...
plot.fig3 <- function(delays, params.delays.data, 
  ...
  plot(delay ~ jitter(ifelse(pathogen == 'Norovirus', 1, 2)), data=delays,
  ...
  bcd.xlabels <- lapply(seq(xlim.bcd[1], xlim.bcd[2], by=2), function(x) 
         bquote(10^.(x)))
...
Differences between symptom onsets reported in Atmar 2008 and Ge 2023.
```{r}
...
combinedNoroData <- combinedNoroData[c(1, 7, 8, 9, 10, 11, 12, 2, 3, 4, 5, 6),]
...
mtext("Day of symptom onset", side=2, line=2.5)
legend(7, 5, pch=c(1, 1, 16), col=c(atmarColor, geColor, 'black'), 
       legend=c(bquote("Atmar" ~ italic("et al.") ~ 2008), 
                bquote("Ge" ~ italic("et al.") ~ 2023),
                "Peak viral shedding"))
