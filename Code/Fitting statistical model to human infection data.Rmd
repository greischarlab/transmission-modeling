...
colnames(noro.measures) <- c('ID', 'dose', 'delay_conservative', 'peak_time', 
                             'shed_onset', 'delay_nonconservative', 'incubation')
...
lm.fit.p2.shedonset.noro <- lm(shed_onset ~ p2, 
                                           data=noro.excluding.unclear.peaks)
...
lm.fit.p2.shedonset.sars <- lm(shed_onset ~ p2, 
                                           data=sars.excluding.unclear.peaks)
...
shedonset.projected_values.noro <- matrix(NA, nrow = iters, ncol = n_points)

colnames(shedonset.projected_values.noro) <- paste("r_", noro.r.range, sep="")
...
shedonset.projected_values.sars <- matrix(NA, nrow = iters, ncol = n_points)

colnames(shedonset.projected_values.sars) <- paste("r_", sars.r.range,
                                                            sep="")
...
for (i in 1:iters) {
  ...
  delay.conservative.resamp.noro <- noro.excluding.unclear.peaks[
    sample(1:nrow(noro.excluding.unclear.peaks), 
                         ...
                         replace=T), c("delay_conservative", "p2")]
  ...
  delay.nonconservative.resamp.noro <- noro.excluding.unclear.peaks[
    sample(1:nrow(noro.excluding.unclear.peaks),
                         size=nrow(noro.excluding.unclear.peaks),
                         replace=T), c("delay_nonconservative", "p2")]
  ...
  peakshedding.projected_values.sars[i, ] <- predict(peakshedding.model.sars,
   ...
   # Fig S5C: shedding onset vs. viral replication rate
  shedonset.resamp.noro <- noro.excluding.unclear.peaks[
    sample(1:nrow(noro.excluding.unclear.peaks),
                         size=nrow(noro.excluding.unclear.peaks),
                         replace=T), c("shed_onset", "p2")]

  shedonset.model.noro <- lm(shed_onset ~ p2, data=shedonset.resamp.noro)
  
  shedonset.projected_values.noro[i, ] <- predict(shedonset.model.noro,
                                                     newdata = data.frame(
                                                       p2 = noro.r.range
                                                     ))
  
  shedonset.resamp.sars <- sars.excluding.unclear.peaks[
    sample(1:nrow(sars.excluding.unclear.peaks),
                         size=nrow(sars.excluding.unclear.peaks),
                         replace=T), c("shed_onset", "p2")]

  shedonset.model.sars <- lm(shed_onset ~ p2, data=shedonset.resamp.sars)
  
  shedonset.projected_values.sars[i, ] <- predict(shedonset.model.sars,
                                                     newdata = data.frame(
                                                       p2 = sars.r.range
                                                     ))
...
# Fig S5C
lower.bound.shedonset.points.noro <- 
  apply(shedonset.projected_values.noro,
        MARGIN = 2, FUN = function(c) quantile(c, 0.025))

upper.bound.shedonset.points.noro <- 
  apply(shedonset.projected_values.noro,
        MARGIN = 2, FUN = function(c) quantile(c, 0.975))

lower.bound.shedonset.points.sars <- 
  apply(shedonset.projected_values.sars,
        MARGIN = 2, FUN = function(c) quantile(c, 0.025))

upper.bound.shedonset.points.sars <- 
  apply(shedonset.projected_values.sars,
        MARGIN = 2, FUN = function(c) quantile(c, 0.975))
...
```{r, fig.height = 7.5, fig.width = 3.5}
measures.combined <- rbind(noro.measures[, c(1:13)], sars.measures)
...

{
  ...
  plot(shed_onset ~ p2, data=measures.combined[!measures.combined$ID %in% 
                                              c(noro.unclearpeaks, sars.unclearpeaks),],
     col=ifelse(pathogen == 'Norovirus', noro.color, sars.color),
     xlim=xlim.bcd,
     ylim=ylim.c, cex.axis=cex, cex=cex,
  ...
  # Lines of best fit
  noro.cpred <- data.frame(p2 = noro.r.range)
  noro.cpred <- transform(noro.cpred, y_predicted = predict(lm.fit.p2.shedonset.noro, newdata=noro.cpred))
  lines(y_predicted ~ p2, data=noro.cpred, col=noro.color)
  
  sars.cpred <- data.frame(p2 = sars.r.range)
  sars.cpred <- transform(sars.cpred, y_predicted = predict(lm.fit.p2.shedonset.sars, newdata=sars.cpred))
  lines(y_predicted ~ p2, data=sars.cpred, col=sars.color)
  
  # Confidence bounds 
  polygon(c(noro.r.range, rev(noro.r.range)), c(upper.bound.shedonset.points.noro,
                                                rev(lower.bound.shedonset.points.noro)), 
        col=rgb(135, 26, 110, alpha=100,
                maxColorValue=255), border = NA)
  
  polygon(c(sars.r.range, rev(sars.r.range)), c(upper.bound.shedonset.points.sars,
                                                rev(lower.bound.shedonset.points.sars)), 
          col=rgb(86, 179, 233, alpha=100,
                maxColorValue=255), border = NA)
...

{
  ...
  plot(peak_time/24 ~ jitter(ifelse(dose == 4.8, .73,
                        ifelse(dose == 48, 1.067, 
                          ifelse(dose == 4800, 1.4, 0)))), 
     data=noro.excluding.unclear.peaks,
     bty='n', ylab="",
