...
We saved the results of our first iteration in a csv file.
```{r}
...
We then plot the trajectories of each participant from both studies.
...
We also note incubation period, shedding onset, and time of peak pathogen load.
...
To assess the relationship between pre-symptomatic transmission and viral replication
rate, we store the projected values from linear regressions between various
measures and replication rate. 

We also use bootstrapping to create 95% confidence intervals (n = 1000) around 
the linear regressions we show in Figures 3 and S6. For each iteration, we 
resample for each participant 1 fit from the 1000 executed previously weighted by
SSE, run a linear regression between the response variable and viral replication 
rate across all participants, and project the response variable values for 10 
evenly-spaced replication rates (log10 scale). After 1000 iterations, we then 
take the 97.5 and 2.5 quantiles of these 10 response variable values. 
```{r} 
...
decay rate and between viral replication rate and dose.
```{r}
...
minus symptom onset). We also break down this relationship by plotting
shedding onset then symptom onset against viral replication rate.
...
Lastly, we plot replication rate, time of peak viral load, and time
