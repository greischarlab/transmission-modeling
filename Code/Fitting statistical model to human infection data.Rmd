...
model parameters plus infectious dose and the duration of the latency period. 
With this non-conservative estimate of pre-symptomatic transmission, we find no 
...
noro.r.range <- seq(min(noro.measures[!noro.measures$ID %in% noro.unclearpeaks,]$p2),
                    max(noro.measures[!noro.measures$ID %in% noro.unclearpeaks,]$p2),
                    length=n_points)
...
noro.latency.range <- seq(min(noro.measures
                              [!is.na(noro.measures$presymp_transmission_avg),]$latency),
                          max(noro.measures
                              [!is.na(noro.measures$presymp_transmission_avg),]$latency),
                          length=n_points)

presymp.shed.avg.projected_values.noro <- matrix(NA, nrow = iters, ncol = n_points)

colnames(presymp.shed.avg.projected_values.noro) <- paste("latency_", noro.latency.range,
                                                          sep="")

presymp.shed.cum.projected_values.noro <- matrix(NA, nrow = iters, ncol = n_points)

colnames(presymp.shed.cum.projected_values.noro) <- paste("latency_", noro.latency.range,
                                                          sep="")

sars.r.range <- seq(min(sars.measures[!sars.measures$ID %in% sars.unclearpeaks,]$p2),
                    max(sars.measures[!sars.measures$ID %in% sars.unclearpeaks,]$p2),
                    length=n_points)
...
sars.latency.range <- seq(min(sars.measures
                              [!is.na(sars.measures$presymp_transmission_avg),]$latency),
                          max(sars.measures
                              [!is.na(sars.measures$presymp_transmission_avg),]$latency),
                          length=n_points)
...
colnames(presymp.shed.avg.projected_values.sars) <- paste("latency_", sars.latency.range,
                                                            sep="")

presymp.shed.cum.projected_values.sars <- matrix(NA, nrow = iters, ncol = n_points)

colnames(presymp.shed.cum.projected_values.sars) <- paste("latency_", sars.latency.range,
                                                            sep="")
...
for (i in 1:iters) {
  ...
  presymp.shed.avg.model.noro <- lm(presymp_transmission_avg ~ latency, data = 
                                      presymp.shed.avg.resamp.noro)
  
  presymp.shed.avg.projected_values.noro[i, ] <- predict(presymp.shed.avg.model.noro,
                                                         newdata = data.frame(
                                                           latency = noro.latency.range
                                                         ))
  ...
  presymp.shed.avg.model.sars <- lm(presymp_transmission_avg ~ latency, data = 
                                      presymp.shed.avg.resamp.sars)
  
  presymp.shed.avg.projected_values.sars[i, ] <- predict(presymp.shed.avg.model.sars,
                                                         newdata = data.frame(
                                                           latency = sars.latency.range
                                                         ))

  # Fig S6: cumulative pre-symptomatic shedding vs. duration of latency period
  ...
  presymp.shed.cum.model.noro <- lm(presymp_transmission_cum ~ latency,
  ...
  presymp.shed.cum.model.sars <- lm(presymp_transmission_cum ~ latency,
...
lower.bound.presymp.shed.avg.points.noro <- 
  apply(presymp.shed.avg.projected_values.noro,
        MARGIN = 2, FUN = function(c) quantile(c, 0.025, na.rm=T))

upper.bound.presymp.shed.avg.points.noro <- 
  apply(presymp.shed.avg.projected_values.noro,
        MARGIN = 2, FUN = function(c) quantile(c, 0.975, na.rm=T))

lower.bound.presymp.shed.avg.points.sars <- 
  apply(presymp.shed.avg.projected_values.sars,
        MARGIN = 2, FUN = function(c) quantile(c, 0.025, na.rm=T))

upper.bound.presymp.shed.avg.points.sars <- 
  apply(presymp.shed.avg.projected_values.sars,
        MARGIN = 2, FUN = function(c) quantile(c, 0.975, na.rm=T))
...

{
  ...
  # Confidence bounds 
  polygon(c(noro.latency.range, rev(noro.latency.range)), 
          c(upper.bound.presymp.shed.avg.points.noro, 
            rev(lower.bound.presymp.shed.avg.points.noro)), 
          col = rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
  
  polygon(c(sars.latency.range, rev(sars.latency.range)), 
          c(upper.bound.presymp.shed.avg.points.sars, 
            rev(lower.bound.presymp.shed.avg.points.sars)), 
          col=rgb(86, 179, 233, alpha=100,
                  maxColorValue=255), border = NA)
...
noro.r.range.nonconservative <- seq(min(noro.measures$p2), max(noro.measures$p2),
                    length=n_points)

sars.r.range.nonconservative <- seq(min(sars.measures$p2), max(sars.measures$p2),
                                    length=n_points)

tiff("Non-conservative pre-symptomatic transmission analysis.tiff", res=500, height=6.5, width=3.5, unit="in")

par(mfrow=c(4, 1), oma=c(1, 2, 0.5, 0), mai=c(0.5, 0.5, 0.1, 0.1))
  ...
  
{
  ...
  # Lines of best fit
  noro.bpred <- data.frame(p2 = noro.r.range.nonconservative)
  noro.bpred <- transform(noro.bpred, y_predicted = predict(lm.fit.p2.delay.nonconservative.noro, newdata=noro.bpred))
  lines(y_predicted ~ p2, data=noro.bpred, col=noro.color)
  
  sars.bpred <- data.frame(p2 = sars.r.range.nonconservative)
  sars.bpred <- transform(sars.bpred, y_predicted = predict(lm.fit.p2.delay.nonconservative.sars, newdata=sars.bpred))
  lines(y_predicted ~ p2, data=sars.bpred, col=sars.color)
  
  # Confidence bounds 
  polygon(c(noro.r.range.nonconservative, rev(noro.r.range.nonconservative)), c(upper.bound.delay.nonconservative.points.noro, 
                                                rev(lower.bound.delay.nonconservative.points.noro)), 
          col = rgb(135, 26, 110, alpha=100,
                  maxColorValue=255), border = NA)
  
  polygon(c(sars.r.range.nonconservative, rev(sars.r.range.nonconservative)), c(upper.bound.delay.nonconservative.points.sars, 
...
Lastly, we plot replication rate (p2) against dose for the norovirus 
study participants. There is no noticeable pattern here.
```{r}
tiff("Replicataion rate vs. dose.tiff", res=500, 
     height=3.5, width=3.5, unit="in")

plot(p2 ~ ifelse(dose == 0.48, 0, 
                 ifelse(dose == 4.8, 0.33, 
                        ifelse(dose == 48, 0.67, 1))), data=noro.measures,
     bty='n', ylab="",
     xlab="", col=noro.color,
     cex=1.1, mgp=c(0, 0.5, 0), tck=-0.04)
mtext(text=expression(Viral~replication~rate~(hour^-1)), side=2, line=1.5)
mtext(text="Infectious dose\n(norovirus, RT-PCR units)", side=1, line=2.75)
