...
infectious dose. We also run a linear regression between viral growth and decay 
rate. In the Ge *et al.*, 2023 study, p1, p2, and p3, i.e. peak viral load, 
...
lm.fit.delay.conservative.dose.noro <- lm(delay_conservative ~ dose, data=noro.measures)
...
# Panel A: Delay between symptom onset and peak viral shedding 
{
  ...
  plot(delay_conservative ~ ifelse(pathogen == 'Norovirus', 
                                   ifelse(dose == 0.48, 0.4, 
                                          ifelse(dose == 4.8, .73,
...
{  
  ...
  plot(delay_nonconservative ~ ifelse(pathogen == 'Norovirus', 
                                   ifelse(dose == 0.48, 0.4, 
                                          ifelse(dose == 4.8, .73,
       ...
       xlab = "",
       mgp=c(0, 0.5, 0), tck=-0.04, las=2, bty='l')
    axis(1, at=c(0.4, 0.73, 1.067, 1.4, 2), 
       labels=c('0.48', '4.8', '48', '4800', '10'), 
       cex.axis=cex, 
     mgp=c(0, 0.5, 0), tck=-0.04)
  ...
  #Average delays per pathogen
  segments(0.3, mean(noro.measures[noro.measures$dose == .48,]$delay_nonconservative, 
                      na.rm=T), 0.5,
           mean(noro.measures[noro.measures$dose == .48,]$delay_nonconservative, 
                na.rm=T), col=noro.color,
           lwd=cex)
  
  segments(0.63, mean(noro.measures[noro.measures$dose == 4.8,]$delay_nonconservative, 
                      na.rm=T), 0.83,
           mean(noro.measures[noro.measures$dose == 4.8,]$delay_nonconservative, 
                na.rm=T), col=noro.color,
           lwd=cex)
  
  segments(0.967, mean(noro.measures[noro.measures$dose == 48,]$delay_nonconservative, 
                     na.rm=T), 1.167,
           mean(noro.measures[noro.measures$dose == 48,]$delay_nonconservative, 
                na.rm=T), col=noro.color,
           lwd=cex)
  
  segments(1.3, mean(noro.measures[noro.measures$dose == 4800,]$delay_nonconservative, 
                     na.rm=T), 1.5,
           mean(noro.measures[noro.measures$dose == 4800,]$delay_nonconservative, 
                na.rm=T), col=noro.color,
           lwd=cex)
  
  segments(1.9, mean(sars.measures$delay_nonconservative, na.rm=T), 2.1,
           mean(sars.measures$delay_nonconservative, na.rm=T), col=sars.color,
           lwd=cex)
...
par(mfrow=c(3, 1), oma=c(4, 2, 0.5, 0), mai=c(0.2, 0.3, 0.2, 0.1))
...
# Viral replication rate vs. dose
{
  ...
  ylabels <- lapply(seq(ylim[1], ylim[2], length.out=5), function(x) 
         bquote(10^.(x))) 
  plot(p2 ~ ifelse(dose == 4.8, 0, 
                 ifelse(dose == 48, 0.5, 
                        ifelse(dose == 4800, 1, 0.9))), 
     data=noro.excluding.unclear.peaks,
     bty='n', ylab="",
     xlab="", col=noro.color,
     xaxt='n', yaxt='n', ylim=ylim, cex=cex)
  axis(1, at=c(0, 0.5, 1), labels = c(4.8, 48, 4800), mgp=c(0, 0.5, 0), tck=-0.04,
       cex.axis=cex)
  axis(2, at=seq(ylim[1], ylim[2], length.out=5),
       labels=do.call(expression, ylabels),
       mgp=c(0, 0.5, 0), tck=-0.04, cex.axis=cex)
  mtext(text=expression(Viral~replication~rate~(hour^-1)), side=2, line=2.5,
        cex=cex_labels)
...

{
  ...
  plot(peak_time/24 ~ ifelse(dose == 4.8, 0, 
                 ifelse(dose == 48, 0.5, 
                        ifelse(dose == 4800, 1, 0.9))), 
     data=noro.excluding.unclear.peaks,
     bty='n', ylab="",
  ...
  axis(1, at=c(0, 0.5, 1), labels = c(4.8, 48, 4800), mgp=c(0, 0.5, 0), tck=-0.04,
       cex.axis=cex)
  mtext(text="Day of peak shedding", side=2, line=2.5, cex=cex_labels)
...

{
  ...
  plot(incubation ~ ifelse(dose == .48, 0, 
                           ifelse(dose == 4.8, 0.33, 
                                  ifelse(dose == 48, 0.67, 
                                         ifelse(dose == 4800, 1, 0.9)))), 
     data=noro.measures,
     bty='n', ylab="",
     ...
     xaxt='n', ylim=ylim,
     cex.axis=cex, cex=cex)
  axis(1, at=c(0, 0.33, 0.67, 1), labels = c(0.48, 4.8, 48, 4800), mgp=c(0, 0.5, 0), 
       tck=-0.04, cex.axis=cex)
  mtext(text="Day of symptom onset", side=2, line=2.5, cex=cex_labels)
...
png("test trajectories ge.png", res=500,
      height=7.5, width=6.5, unit="in")

par(mfrow=c(6, 3), mai=c(0.22, 0.42, 0.22, 0.22), oma=c(3.5, 4, 0, 0),
    mar=c(1, 2.1, 1, 1))

for(i in 1:length(noroParticipants[noroParticipants != 'ID20'])) {
  
  p <- noroParticipants[i]
  participantdata <- noro.FShedData[noro.FShedData$ID==p, ]
  
  participantgedata <- gedata[gedata$ID == p,]
  ...
  plot(1, col=col, xlim=c(0, 25), 
       ylim=c(7, 15), xlab="", ylab="", xaxt="n", yaxt="n", cex.lab=1.33, 
       cex.axis=1.33, font.lab=2, bty='n')
  
  points(participantdata$days, log10(participantdata$y_virus), col=col, 
         cex=1.33, pch=pch)
  
  linet <- seq(min(participantdata[!is.na(participantdata$y_virus),]$days) * 24, 
                 participantdata[maxt,]$days * 24, by=1)

  predictedvl <- generate.estimates(linet, params)

  lines(linet / 24, predictedvl, col=col)
  
  lines(participantgedata$days, log10(exp(participantgedata$Estimate)), col="blue")
  ...
  xLabs = c(expression(0),
                  ...
                  expression(20),
                  expression(25))
  axis(1, cex.axis=1, at=seq(0, 25, by=5), labels = xLabs,
       line=1.25,
       mgp=c(2, 0.75, 0), tck=-0.04)
  ...
  axis(2, cex.axis=1, at=seq(3, 15, by=3), 
       labels=yLabs,
       line=1.25,
       las=2, 
       mgp=c(2, 1, 0), tck= -0.04)
  ...
  text(20, 12, p, font=2)
  text(20, 10.5, participantgedata$dose_cat, font=2)
}
dev.off()
```
