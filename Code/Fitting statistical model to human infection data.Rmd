...
first.local.min <- function(data) {
  ...
  minima <- local.min.max(data[!is.na(data$y_virus),]$y_virus)$minima

  for (mi in minima) {
    
    min.index <- which(data$y_virus == mi)
    data.context <- data$y_virus[(min.index - 2): (min.index + 2)]
    if(length(na.omit(data.context)) == 5 & (min(data.context) == mi)) {
      if(sum(!is.na(data$y_virus[1:min.index])) < 5) {
        non.na.indexes <- which(!is.na(data$y_virus))
        return(non.na.indexes[5])
      } else {
        return(min.index)
      }
    }
  ...
  return(max(which(!is.na(data$y_virus))))
  
}

back.to.initial <- function(data) {

  less.than.initial <- which(data$y_virus[2:length(data$y_virus)] <= data$y_virus[1])

  if (length(less.than.initial) == 0) {
    return(max(which(!is.na(data$y_virus))))
  } else {
    if(sum(!is.na(data$y_virus[1:less.than.initial])) < 5) {
      non.na.indexes <- which(!is.na(data$y_virus))
      return(non.na.indexes[5])
    } else {
      return(min(less.than.initial))
    }
...
plot.viralload <- function(data, pars, sympdata, max.modeltime, cex=1,
  ...
  # Plotting real VL
  plot(log10(y_virus) ~ days, data=data, col=col, xlim=c(0, 25), 
       ylim=c(ylim.lower, 15), xlab="", ylab="", xaxt="n", yaxt="n", cex.lab=cex, 
       cex.axis=cex, pch=pch, font.lab=2, cex=cex, bty='n')
  
  # Plotting simulated VL
  linet <- seq(min(data[!is.na(data$y_virus),]$days) * 24, data[max.modeltime,]$days * 24, by=1)
...
participantexamples <- c('Throat_5', 'ID6', 'Throat_16')
...
tiff("test Participant Trajectories Norovirus.tiff", res=500,
...
```{r}
tiff("test Participant Trajectories SARS-CoV-2.tiff", res=500,
      height=7.5, width=6.5, unit="in")
...
noroParticipants <- unique(noro.FShedData[!(noro.FShedData$ID %in% c('ID20', 'ID17', 'ID4', 'ID5')),]$ID)
...
colnames(delays.conservative) <- c('ID', 'delay')
```
...
colnames(delays.nonconservative) <- c('ID', 'delay')
```
...
colnames(transmission.latent) <- c('ID', 'transmission_potential', 'symptom_onset')
...
colnames(transmission.latent.cum) <- c('ID', 'transmission_potential', 'symptom_onset')
...
colnames(participant.peaktimes) <- c("ID", "peaktime")
...
params.symp.data <- merge(merge(noro.params.localmin, noro.IncubData[, c(1, 5)]),
                          delays.conservative)
...
plot.fig3 <- function(delays, params.delays.data, 
                      lm.fit.b.noro, lm.fit.b.sars, ylim.ab,
                      transmission.latent.incubation.data=NA, xlim.bcd,
                      lm.fit.c.noro=NA, lm.fit.c.sars=NA,
  ...
  plot(delay ~ ifelse(pathogen == 'Norovirus', 1, 2), data=delays,
       ...
       pch=ifelse(delay < 0 & pathogen == 'Norovirus', 17, 
                  ifelse(delay > 0 & pathogen == 'Norovirus', 2,
                         ifelse(delay < 0 & pathogen == 'SARS-CoV-2',
                                23, 5))),
  ...
  legend(1.1, ylim.ab[2], legend=c("Pre-symptomatic", "Post-symptomatic"), 
  ...
  abline(lm.fit.b.noro, col=noro.color)
  abline(lm.fit.b.sars, col=sars.color)
  ...
  text(panellabelratio * 9 - 5, panellabelratio * (ylim.ab[2] - ylim.ab[1]) + 
  ...
  if (!nonconservative) {
    ...
    axis(1, at=seq(xlim.bcd[1], xlim.bcd[2], by=2), 
       ...
       cex.axis=cex, 
       mgp=c(0, 0.5, 0), tck=-0.04)
    axis(2, at=seq(1,8, by=2), cex.axis=cex, mgp=c(0, 0.5, 0), tck=-0.04, las=2)
    
    abline(lm.fit.p2.peaktime, col=noro.color)
    abline(lm.fit.p2.peaktime.sars, col=sars.color)
    
    mtext(text="Day of \npeak shedding", side=2, line=2, cex=cex_labels)
    
    text(panellabelratio * 9 - 5, panellabelratio * 7 + 1, labels="C", cex=cex, font=2)
      
    # Panel D: Symptom onset vs. viral replication rate
    plot(incubation ~ p2, data=params.symp.data,
         col=ifelse(pathogen == 'Norovirus', noro.color, sars.color), 
         xlim=xlim.bcd, ylim=c(0, 11), cex.axis=cex, cex=cex,
         pch=ifelse(delay < 0 & pathogen == 'Norovirus', 17, 
                    ifelse(delay > 0 & pathogen == 'Norovirus', 2,
                           ifelse(delay < 0 & pathogen == 'SARS-CoV-2',
                                  23, 5))),
    ...
    axis(2, at=seq(0, 11, by=3), cex.axis=cex, mgp=c(0, 0.5, 0), tck=-0.04, las=2)
    
    abline(lm.fit.p2.symponset, col=noro.color)
    abline(lm.fit.p2.symponset.sars, col=sars.color)
    
    mtext(text="Day of \nsymptom onset", side=2, line=2, cex=cex_labels)
    mtext(text=expression(Viral~replication~rate~(hour^-1)),
          side=1, line=2, cex=cex_labels)
    text(panellabelratio * 9 - 5, panellabelratio * 11, labels="D", cex=cex, font=2)
         ...
         pch=ifelse(is.na(ifelse(delay < 0 & pathogen.y == 'Norovirus', 17, 
                ifelse(delay > 0 & pathogen.y == 'Norovirus', 2,
                       ifelse(delay < 0 & pathogen.y == 'SARS-CoV-2',
                              23, 5)))),
                ifelse(pathogen.y == 'Norovirus', 14, 9), 
    ...
    axis(2, cex.axis=cex, mgp=c(0, 0.5, 0), tck=-0.04, at=seq(0, 15, by=5),
    ...
    abline(lm.fit.c.noro, col=noro.color)
    abline(lm.fit.c.sars, col=sars.color)
    
    text(panellabelratio * 11, panellabelratio * 16 - 1, labels="E", cex=cex, font=2)
  }

}

combined.delays.conservative <- rbind(delays.conservative, delays.conservative.sars)
combined.delays.conservative$pathogen <- ifelse(grepl('ID', 
                                                      combined.delays.conservative$ID), 
                                                'Norovirus', 'SARS-CoV-2')

params.and.delays.conservative.combined <- rbind(params.and.delays.conservative, 
...
transmission.latent.combined <- rbind(transmission.latent, transmission.latent.sars)
transmission.latent.combined$pathogen <- ifelse(grepl('ID', 
                                                      transmission.latent.combined$ID), 
                                                'Norovirus', 'SARS-CoV-2')

combined.params.peaktime.data <- rbind(params.peaktime.data, params.peaktime.data.sars)
combined.params.peaktime.data$pathogen <- ifelse(grepl('ID', 
                                                      combined.params.peaktime.data$ID), 
                                                'Norovirus', 'SARS-CoV-2')

combined.params.sympdata <- rbind(params.symp.data, params.symp.data.sars)
combined.params.sympdata$pathogen <- ifelse(grepl('ID', 
                                                      combined.params.sympdata$ID), 
                                                'Norovirus', 'SARS-CoV-2')

tiff("Figure 3.tiff", res=500, height=6.5, width=3.5, unit="in")
...
combined.delays.nonconservative <- rbind(delays.nonconservative, delays.nonconservative.sars)
combined.delays.nonconservative$pathogen <- ifelse(grepl('ID', 
                                                      combined.delays.nonconservative$ID), 
                                                'Norovirus', 'SARS-CoV-2')

params.and.delays.nonconservative.combined <- rbind(params.and.delays.nonconservative, 
...
tiff("Figure S4.tiff", res=500, height=6.5, width=3.5, unit="in")
