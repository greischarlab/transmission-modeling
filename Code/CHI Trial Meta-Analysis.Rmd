...
plot.segment <- function(xstart, col, ypos, cex, symptoms, isindividual, citation) {
  ...
  mtext(citation, side=2, cex=0.8, las=2, at=ypos, line=0.5)
...
plot.pane <- function(dat, axis.labels) {
  ...
  plot(0, 0, xlim=c(0, 6), ylim=c(0, max(nrow(rightpane), nrow(leftpane)) * 4) + 3, 
       col = "white", xlab= "", ylab='', yaxt='n',
       xaxt="n")
  
  axis(1, at=seq(0, 6, by=1), mgp=c(0, 0.4, 0), tck=-0.02, cex.axis=0.8)
  axis.ystart <- 0
...
chidata <- read.csv(here('data', 'ViralCHIStudies.csv'))
...
tiff("CHI trial literature review.tiff", res=500, height=6.5, width=6.5, unit="in")
{
  par(bty='n', mfrow=c(1, 2), oma=c(1, 3, 0, 0), mar=c(1.1, 7.1, 0, 0))
