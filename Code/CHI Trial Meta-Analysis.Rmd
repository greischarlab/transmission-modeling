...
plot.segment <- function(xstart, col, ypos, cex, symptoms, isindividual, citation) {
  ...
  if (isindividual) {
    ...
    text(xstart + 1.2, ypos, citation, pos=4, cex=0.8)
  } else {
    text(xstart + 0.2, ypos, citation, pos=4, cex=0.8)
  }
...
plot.pane <- function(dat, axis.labels) {
  ...
  plot(0, 0, xlim=c(0, 20), ylim=c(0, max(nrow(rightpane), nrow(leftpane)) * 4) + 3, 
       col = "white", xlab= "", ylab='', yaxt='n',
       xaxt="n")
  
  axis(1, at=seq(0, 20, by=5), mgp=c(0, 0.4, 0), tck=-0.02, cex.axis=0.8)
  axis.ystart <- 0
  ...
  for (v in 1:length(unique(dat$virus))) {
    ...
    for (i in 1:nrow(currvirusdat)) {
      ...
      text(-0.5, axis.yend + 1, axis.labels[v], col=currviruscol, pos=4,
           cex=0.8)
...
{
  par(bty='n', mfrow=c(1, 2), oma=c(0.1, 0, 0, 0), mai=c(0.52, 0, 0, 0.02))
  plot.pane(leftpane, labels.left)
  mtext('Days post-inoculation', side=1, line=-1, outer=TRUE,
        adj=0.5, cex=0.8)
  ...
  legend(x=12, y=15,
         ...
         pt.cex=c(1.8, 0.9, 1), y.intersp=1.5, cex=0.75, bty="n")
}
dev.off()
```
