...
Our packages for ODE model and plotting:
...
line depicting the threshold for symptoms. We are defining pre-symptomatic 
...
{
  tiff(filename="Model Schematics.tiff", height=7.5, width=6.5,
      units="in", res=500)
...
tiff(file="Delays varying r.tiff",
           height=3.5, width=3.5, units="in", res=500)
grid.arrange(plot.varyingr.nocap, plot.varyingr.cap, nrow=2, 
             left=textGrob("Delay between symptom onset \n and peak pathogen load",
             gp = gpar(fontsize = 6),
             rot=90))
dev.off()
```
...
{
  tiff(filename="Pathogen and immune peak times.tiff", height=3.5, width=6.5,
      units="in", res=500)
  
  par(mfrow=c(1, 2), mai=c(0.82, 0.72, 0.52, 0.06))
  
  plot(pathogen.peaktime ~ r, data=peaktimes.cap, ylim=c(0, 0.15), type="l",
       xlab="", ylab="Time of max abundance", bty="l", mgp=c(2.25, 0.75, 0))
  lines(immune.peaktime ~ r, data=peaktimes.cap, col="red")
  ...
  plot(pathogen.peaktime ~ r, data=peaktimes.nocap, ylim=c(0, 0.15), type="l",
       xlab="", ylab="", bty="l")
  lines(immune.peaktime ~ r, data=peaktimes.nocap, col="red")
