...
symptomatic.medr.lowthresh <- symptomatic.df(medr, 'low')
symptomatic.medr.highthresh <- symptomatic.df(medr, 'high')

highr <- varyingr[varyingr$r == r_variable[3],] 
params.highr <- get.params(gamma_variable[2], r_variable[3])
symptomatic.highr.lowthresh <- symptomatic.df(highr, 'low')
symptomatic.highr.highthresh <- symptomatic.df(highr, 'high')
...
plot.varyingr <- function(lowr.df, medr.df, highr.df, symp.lowr.df, symp.medr.df, symp.highr.df) {
  ...
  xlim(0, 1.8) +
  geom_line(data=lowr.df, aes(colour=toString(lowr.df$r[1]))) +
  geom_line(data=medr.df, aes(colour=toString(medr.df$r[1]))) +
  geom_line(data=highr.df, aes(colour=toString(highr.df$r[1]))) +
  facet_wrap(~ dose, ncol=1, labeller=labeller(dose=idlabels)) +
  
  #Bolded transmission potential lines where symptoms occur
  geom_line(data=symp.lowr.df, aes(colour=toString(lowr.df$r[1])), lwd=1.5) +
  geom_line(data=symp.medr.df, aes(colour=toString(medr.df$r[1])), lwd=1.5) +
  geom_line(data=symp.highr.df, aes(colour=toString(highr.df$r[1])),lwd=1.5) +
