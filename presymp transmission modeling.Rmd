...
varyinggamma <- read.csv("Varyinggammas.csv")
...
delays.bygamma.lowsympthresh <- read.csv("Delays by gamma low symp thresh.csv")
delays.bygamma.medsympthresh <- read.csv("Delays by gamma med symp thresh.csv")
delays.bygamma.highsympthresh <- read.csv("Delays by gamma high symp thresh.csv")
```
...
freqsymp.bydose.lowsympthresh <- read.csv("Frequency of Presymptomatic Transmission by Dose (low symptom threshold, group population at 100).csv")

freqpresymp.bydose.medsympthresh <- read.csv("Frequency of Presymptomatic Transmission by Dose (low symptom threshold, group population at 100).csv")

freqpresymp.bydose.highsympthresh <- read.csv("Frequency of Presymptomatic Transmission by Dose (low symptom threshold, group population at 100).csv")
```
...
plot.varyingr <- function(delaysbyr, symp.thresh, pointsize, textsize) {
  ggplot(delaysbyr, aes(x=r, y=delay.peakload.symponset)) + 
  geom_point(col="#E69F00", size=pointsize) +
  ylab("Delay Between Peak \n Pathogen Load \n and Symptom Onset") +
  xlab("Replication Rate (r)") +
  theme(axis.text= element_text(size=textsize), axis.title= element_text(size=textsize), panel.background=element_rect(fill="white", colour="black"))
}

plot.varyingr(delays.byr.lowsympthresh, low.sympthresh)
plot.varyingr(delays.byr.medsympthresh, med.sympthresh)
plot.varyingr(delays.byr.highsympthresh, high.sympthresh, 10, 30)
...
plot.varyingdose <- function(presympfreq.bydose, symp.thresh, pointsize, textsize, col) {
  ggplot(presympfreq.bydose, aes(x=dose, y=freq.presymp)) + geom_point(color= col, size=pointsize) +
  xlab("Infectious Dose") + 
  ylab("Frequency of \n Pre-symptomatic \n Transmission") + 
  theme(axis.text= element_text(size=textsize), axis.title= element_text(size=textsize), panel.background=element_rect(fill="white", colour="black")) +
  scale_x_log10()
}; plot.varyingdose(freqpresymp.bydose.highsympthresh, high.sympthresh, 10, 30, cols[3])
...
plot.dynamics <- function(df, point, symp.thresh, var_name, k) {
  size=2
  cols= c("Transmission Potential"= "black", "Immune Response" = "purple", 
          "Symptom Threshold" = "red", "Peak Pathogen Load" = "darkgoldenrod1")
  
  linetypes= c("Transmission Potential"= "solid", "Immune Response" = "solid", 
          "Symptom Threshold" = "dashed", "Peak Pathogen Load" = "solid")
  df = df[df[var_name] == point,]
  
  ggplot(df, 
         aes(x=time, y=log10(transmission_potential))) + 
  geom_line(aes(color="Transmission Potential", linetype="Transmission Potential"), size=size) + 
  geom_line(aes(y=log10(immune), 
             color="Immune Response", linetype="Immune Response"), size=size) + 
  geom_hline(aes(yintercept=log10(symp.thresh),
             color="Symptom Threshold", linetype="Symptom Threshold"), size=size) +
  geom_vline(aes(xintercept=peak.pathogenloadtime(df, k), 
             color="Peak Pathogen Load", linetype="Peak Pathogen Load"),
             show.legend = FALSE, size=size) +
  
  xlab("Time") + ylab("Transmission Potential (log10)") + xlim(0.3, 0.8) + 
  scale_y_continuous(limits=c(0, 2.1), sec.axis=sec_axis(~. *1, name="Immune Response (log10)")) +
  scale_color_manual(name="",values = cols) +
  scale_linetype_manual(name="",values = linetypes) 
}

#Points at which presymptomatic frequency oscillated in the varying gamma plot
...
plot.dynamics(varyinggamma, points[1], high.sympthresh, "gamma", k)

#This is the point (gamma value) at which transmission potential was at 0.001 (peak) 
plot.dynamics(varyinggamma, points[2], high.sympthresh, "gamma", k)

#Transmission potential is still at 0.001 (peak) before oscillating back down to 0
plot.dynamics(varyinggamma, points[3], high.sympthresh, "gamma", k)

#Transmission potential oscillates back down to 0
plot.dynamics(varyinggamma, points[4], high.sympthresh, "gamma", k)
