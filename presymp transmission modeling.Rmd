...
df <- function(id, time, params) {
  df= data.frame(lsoda(c(P=id, X=0), time, transmission.model, params))
...
plot.immune.response.panel <- function(ids, params, cols, label, time) {
  datalow= df(ids$low, time, params)
  datamed= df(ids$med, time, params)
  datahigh= df(ids$high, time, params)
...
On 11/10/21, I constructed a function to determine peak immune response for applicable combinations of r and gamma values. Using this value, I then determined three thresholds for determinine symptoms--above peak immune response, below, and right at it.
```{r}
peak.immuneresponse <- function(id, time, params) {
  data = df(id, time, params)
  data = max(data$X)
}
```
